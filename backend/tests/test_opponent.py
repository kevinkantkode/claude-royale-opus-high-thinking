...
def test_record_play_mirror_copies_last_card(cards):
    ...
    with patch("game.opponent.time.time", side_effect=times):
        start_game()
        record_play("knight", cards)  # Knight costs 3 -> 7.5 - 3 = 4.5
        state = record_play("mirror", cards)  # Mirror copies knight, costs 4. 4.5 + 2.14 - 4 ~ 2.6
...
def test_record_play_not_enough_elixir_raises(cards):
    ...
    start_game()
    record_play("skeletons", cards)  # Drain to 6.5 (7.5 - 1)
    with pytest.raises(ValueError, match="Not enough elixir"):
        record_play("mega-knight", cards)  # Mega Knight costs 7, we have 6.5
...
def test_sync_game_success(cards):
...
def test_sync_game_already_used_raises(cards):
...
def test_sync_game_too_late_raises(cards):
